...
// Define types for classroom and student
interface Classroom {
  _id: string;
  title: string;
  description: string;
...
export function AnimatedTooltipPreview() {
  const { classCode } = useParams(); // Assuming you are using Next.js for routing
  ...
  useEffect(() => {
    const fetchStudentsData = async () => {
      if (classroomData && classroomData.students.length > 0) {
        const studentIds = classroomData.students.map(student => student._id);
        try {
          const response = await axios.get<Student[]>(
            `${BASE_URL}/user/getStudentsData`,
            {
  ...
  if (!classroomData) {
    return <Loading />; // Add a loading indicator while fetching data
  }


  const people = students.map(
  ...
  return (
    <div className="flex flex-row  justify-end w-[96%]   right-6">
